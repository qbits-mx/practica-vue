<template>
 <footer class="page-footer why">

 <nav class="navbar navbar-expand-lg navbar-light bg-light">

<div class="collapse navbar-collapse " id="navbarNavAltMarkup">
  <div class="navbar-nav">
    <a class="nav-item nav-link" href="#">Trabaja con nosotros</a>
    <a class="nav-item nav-link" href="#"> Termino y condiciones</a>
    <a class="nav-item nav-link" href="#"> Como cuidamos tu privacidad</a>
    <a class="nav-item nav-link " href="#"> Accesibilidad</a>
    <a class="nav-item nav-link " href="#"> Ayuda</a>
  </div>
</div>

</nav>
<small>Copyright © 1999-2023 El presente canal de instrucción o ambiente, es operado por DeRemate.Com de México, S. de R.L. de C.V. identificada bajo la marca comercial "Mercado Libre".
Insurgentes Sur 1602 Piso 9 Suite 900, Crédito Constructor Benito Juarez, 03940 Ciudad de México, CDMX, Mexico 
</small>
</footer>


</template>

<style>
.why{
  background: white;
  margin-top: 700px;
}

</style>
